...
impl NotificationService {
    ...
    pub async fn notify(&self, user_id: Uuid, event: NotificationEvent, payload: serde_json::Value) -> anyhow::Result<()> {
        // Store in DB
        sqlx::query(
        ...
        let mut attempts = 0;
        while attempts < 3 {
            match self.email_sender.send("user@example.com", "Notification", &format!("{:?}", event)).await {
                Ok(_) => break,
                Err(e) => {
                    attempts += 1;
                    if attempts == 3 {
